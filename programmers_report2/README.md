# programmers_report2
## 프로그래머스 고양이 사진 검색 사이트 만들기

'2020 Dev-Matching: 웹 프론트엔드 개발자(상반기)' 기출 문제 내용입니다. -> programmers_report1 은 21년 상반기 기출 문제입니다.

// 출처: 프로그래머스 과제, 고양이 사진 검색 사이트, https://programmers.co.kr/skill_check_assignments/4

### 22.03.07 작업 진행
- 5시간 동안 문제 푼 결과... 다크몯, 이전 검색 결과, 새로고침 제외 작업을 그나마 진행 어느정도 완성을 해냄
- 랜덤 고양이 배너 섹션 추가 -> 해당 부분 같은 경우 버튼을 만들라고, 검색 페이지 관련 문서부분에 되어있는데, 이 버튼을 클릭 시 다른 섹션으로 보여달라고 써있어서
  처음에 검색 결과에서 같이 노출했다가, 다시 변경을 진행 (지문에 맞게 해둔건지는 정확하지 않은 상태)

### 22.03.09 작업 진행
- 나머지 부분 적용 완료
- 체크박스와 css 이용 darkMode 적용 완료 -> DarkMode.js 에 해당 내용 들어있음
- localStorage 를 이용해서, 새로고침 시 마지막에 봤던 결과 저장 및 키워드 값 저장,  랜덤 부분 저장 -> 그 후 호출 할 수 있도록 추가 
