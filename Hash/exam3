...
--------------------------------------------------------------------------------------------------------------------
해당 문제는 노래를 각각 같은 노래로 묶고 play 값이 높은 순으로 나오며, 노래 종류별로 제일 높은 번호를 2개씩 반환하는 문제입니다.
그렇기 때문에 Map에 노래별로 제목을 객체명(key)으로 묶는데 값(value)에 해당 노래의 번호와 play값을 배열로 묶고, 토탈 play값을 따로 묶을 필요가 있습니다.
